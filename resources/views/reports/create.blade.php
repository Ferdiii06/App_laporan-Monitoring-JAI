...
@section('title', 'Report Defect ' . request('type', 'Final Assy'))
...
<div x-data="defectForm()" class="bg-white min-h-[calc(100vh-3.5rem)] flex flex-col">
    ...
    <div class="px-4 py-3 flex items-center border-b border-[#EEEEEE]">
        ...
        <h1 class="text-base font-bold text-[#B71C1C]">Report Defect {{ $type }}</h1>
    </div>

    <!-- MAIN FORM -->
    <form action="{{ route('reports.store') }}" method="POST" id="defectForm" class="flex flex-col flex-grow relative">
        @csrf
        ...
        <div x-show="step === 1" class="flex flex-col flex-grow p-4 space-y-5 pb-24">
            ...
            <div class="flex justify-between items-center mb-1">
                ...
                <span class="text-sm text-[#B71C1C] font-bold">Informasi Dasar</span>
            </div>
        ...
        <div x-show="step === 2" x-cloak class="flex flex-col flex-grow p-4 pb-24">
            <div class="border border-[#DDDDDD] rounded-lg p-4 bg-white shadow-sm mb-4">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-[15px] font-bold text-[#B71C1C]">Detail Laporan Defect</h2>
                ...
                <div class="flex justify-between">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-[15px] font-bold text-black">{{ session('current_shift', Auth::user()->shift ?? '1A') }}</p>
                    </div>
                </div>
            ...
            <div class="bg-[#FFF8F0] p-3 rounded-md flex items-start mb-5">
                ...
                <p class="text-xs text-gray-700 leading-relaxed">Pastikan semua data di atas sudah benar sebelum mengirimkan laporan ke tim pemeliharaan.</p>
            </div>
        </div>
        ...
        <div class="fixed bottom-0 left-0 right-0 max-w-7xl mx-auto px-4 py-3 bg-white border-t border-[#EEEEEE]">
            ...
            <div x-show="step === 2" x-cloak class="space-y-2">
                <button type="submit" class="w-full flex justify-center items-center py-3 px-4 rounded-lg shadow-sm text-sm font-bold text-white bg-[#B71C1C] hover:bg-[#8B0000] focus:outline-none transition-colors">
                    KIRIM LAPORAN <svg class="ml-2 w-4 h-4 flex-shrink-0" style="width: 1.25rem; height: 1.25rem;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg>
                </button>
